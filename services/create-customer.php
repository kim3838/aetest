...
$app::debug(print_r(['CREATE CUSTOMER REQUEST', $request], true));
...
$app::debug(print_r(['CREATE CUSTOMER ERROR', $errors], true));
...
if (!count($errors)) {
    ...
    if (move_uploaded_file($request['file']['tmp_loc'], $relativeStoragePath . $fileName)){
        ...
        $query = "INSERT INTO customers(lastname, firstname, email, city, country, image) VALUES (:lastname,:firstname,:email,:city,:country,:image)";
        ...
        if($statement->rowCount() > 0) {
            echo $app->successfulResponse([], 'Customer created');
            die();
        } else {
            echo $app->serverErrorResponse([], 'Error creating customer');
            die();
        }
    } else {
